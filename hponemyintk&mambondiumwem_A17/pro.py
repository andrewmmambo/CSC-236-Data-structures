 # Modified from Code Written by Christian Stigen Larsen, http://csl.sublevel3.org

import sys

def definiteNoun(s):
  "Add definite form to a noune, for instance 'horse' becomes 'a horse'"
  s = s.lower().strip()
  if s in ['a', 'e', 'i', 'o', 'u', 'y']:
    return "an " + s
  else:
...
tries = 0
Q = (makeQuestion('Does it have 4 legs?', 'Horse', 'beetle'))
q = Q
Q = (makeQuestion('Can it speak?', 'parrot', 'crocodile'))
